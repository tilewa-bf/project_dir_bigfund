...
<head>
    <meta charset="utf-8" />
    <title>Bigfund | Customer Balance Report</title>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <h4 class="card-title" style="margin-top:30px;margin-bottom: -20px;">Daily Balance Report</h4><br/>
                                        ...
                                        <tbody>
                                            ...
                                            if($_SERVER["REQUEST_METHOD"]=="POST"){
                                              if(isset($_POST['submit'])){
                                                ...
                                                $sqlcustbal= "SELECT cust_acc,cust_name , SUM(COALESCE(CASE WHEN entry_type = 'credit' THEN amount END,0)) AS total_credits , SUM(COALESCE(CASE WHEN entry_type = 'debit' THEN amount END,0)) AS total_debits , SUM(COALESCE(CASE WHEN entry_type = 'credit' THEN amount END,0)) - SUM(COALESCE(CASE WHEN entry_type = 'debit' THEN amount END,0)) AS balance FROM dailysavingsmaster_tbl GROUP BY cust_acc HAVING balance <> 0";

                                                $querycustbal = mysqli_query($conn,$sqlcustbal);
                                                while($rows = mysqli_fetch_assoc($querycustbal)){
                                                    $custacc = $rows['cust_acc'];
