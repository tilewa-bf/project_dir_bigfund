                                    ...
                                    <div class="card-body">
                                        ...
                                        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data">
                                            ...
                                            <div class="row">
                                                <div class="col-lg-3">
                                                    <div class="mb-3">
                                                        <label for="formrow-status" class="form-label">Link </label>
                                                        <select name="linkid" id="formrow-group" class="form-select">
                                                            <option value="Choose Status" selected>Choose Link</option>
                                                            <?php
                                                            $sql = "SELECT * FROM link_tbl WHERE status='active' ORDER BY link_name";
                                                            $result = mysqli_query($conn,$sql);
                                                            while($row = mysqli_fetch_assoc($result)){
                                                            $dblinkid = $row['link_id'];
                                                            $dblinkname = $row['link_name'];
                                                             ?>
                                                    <option value="<?php echo $dblinkid;?>"><?php echo $dblinkname;?></option>
                                                    <?php }?>
                                                        </select>
                                                    </div>
                                                </div>
                                                <div class="col-md-3">
                                                    <div class="mb-3">
                                                        <label for="formrow-group-input" class="form-label">Sub Link name</label>
                                                        <input type="text" name="sublinkname" class="form-control" id="formrow-linkname-input" placeholder="Enter Sub Link Name" required>
                                                    </div>
                                                </div>
                                                <div class="col-md-3">
                                                    <div class="mb-3">
                                                        <label for="formrow-group-input" class="form-label">Sub Link Url</label>
                                                        <input type="text" name="sublink_url" class="form-control" id="formrow-linkicon-input" placeholder="Enter Sub Link Url" required>
                                                    </div>
                                                </div>
                                                <div class="col-lg-3">
                                                    <div class="mb-3">
                                            ...
                                            <div>
                                                <button style="margin:0px 0 10px 20px" type="submit" name="submit" class="btn btn-primary waves-effect waves-light">Submit</button>
                                            </div>
                                        </form>
