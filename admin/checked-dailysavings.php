                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <h4 class="card-title" style="margin-top:30px;margin-bottom: -20px;">Customers Balance Report</h4><br/>           
                                        ...
                                        <table id="datatable-buttons" class="table table-bordered dt-responsive nowrap w-100">
                                            ...
                                            if($_SERVER["REQUEST_METHOD"]=="POST"){
                                                   if(isset($_POST['submit'])){
                                                     ...
                                                     $accountofficer=$_POST['fullname'];
                                                    $sql ="SELECT cust_acc,cust_name,account_officer,checked, SUM(COALESCE(CASE WHEN entry_type = 'credit' THEN amount END,0)) AS total_credits , SUM(COALESCE(CASE WHEN entry_type = 'debit' THEN amount END,0)) AS total_debits , SUM(COALESCE(CASE WHEN entry_type = 'credit' THEN amount END,0)) - SUM(COALESCE(CASE WHEN entry_type = 'debit' THEN amount END,0)) AS balance FROM data_entry_tbl WHERE account_officer='$accountofficer' AND checked='checked' GROUP BY cust_acc HAVING balance <> 0";
                                                    $query = mysqli_query($conn,$sql);
                                                    while($rows = mysqli_fetch_assoc($query)){
                                                        $accno = trim($rows['cust_acc']);
                                            ...
                                            <tr>
                                                ...
                                                <td><button id="checker" style="background: <?php echo $color;?>;"><?php echo $text;?></button>
                                                </td>
                                            </tr>
                                        <?php $i++; }
                                                }}?>
        ...
        <script>
            function  checkds(id){
                if(confirm("Daily Savings WTD Checker!")){
                    window.location.href  = "checkdailysavings.php?checkds="+id;
                }
            }
        </script>
</body>
