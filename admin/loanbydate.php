...
<head>
    <meta charset="utf-8" />
        <title>Bigfund | Loan by Date Report</title>
                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <h4 class="card-title" style="margin-top:30px;margin-bottom: -20px;">Loan Granted by Date Report</h4><br/>
                                        ...
                                        <table id="datatable-buttons" class="table table-bordered dt-responsive nowrap w-100">
                                            ...
                                            if($_SERVER["REQUEST_METHOD"]=="POST"){
                                                  if(isset($_POST['submit'])){
                                                    ...
                                                    $sql ="SELECT * FROM loan_request WHERE tenor='WEEKLY' AND approved_date BETWEEN '$from' AND '$to' AND status!='pending' ORDER BY loan_id DESC ";
                                                    $query = mysqli_query($conn,$sql);
                                                    while($rows = mysqli_fetch_assoc($query)){
                                                        $loanno = $rows['loan_code'];
                                                        $accname = $rows['member_name'];
                                                        $applieddate = $rows['app_date'];
                                                        $approveddate = $rows['approved_date'];
                                                        $postedby = $rows['created_by'];
                                                        $appliedamt = $rows['applied_amount'];
                                                        $approvedamt = $rows['approved_amount'];
                                                        $totalpayable=$rows['total_payable'];
                                                        $interest=$totalpayable-$approvedamt;
                                                        $total = $total + $totalpayable;
                                            ?>
                                            <tr>
                                                <td><?php echo $i;?></td>
                                                ...
                                                <td><?php echo strtoupper($accname);?></td>
                                                <td><?php echo strtoupper($postedby);?></td>
                                                
                                                <td><?php echo number_format($appliedamt,2);?></td>
