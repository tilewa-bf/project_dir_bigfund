...
<head>
    <meta charset="utf-8" />
        <title>Geexsmith | Festive Report by Category and Account Officer</title>
                                    ...
                                    <div class="card-body">
                                        ...
                                        <h4 class="card-title" style="margin-top:30px;margin-bottom: -20px;">Loam Mem. by Account Officer</h4><br/>
                                            ...
                                            <div style="margin-top: 20px;" class="row">
                                                <div class="col-xl-3">
                                                    <select name="accoff" id="formrow-inputState" class="form-select" required>
                                                 ...
                                                 <div class="col-xl-3">
                                                    <select name="category" id="formrow-inputState" class="form-select" required>
                                                                <option value="">Select Category</option>
                                                                <?php 
                                                                $sql ="SELECT * FROM festive_category_tbl WHERE status2='undelete' AND status='Active'";
                                                                $result= mysqli_query($conn,$sql);
                                                                while($rows = mysqli_fetch_assoc($result)){
                                        ...
                                        <table id="datatable-buttons" class="table table-bordered dt-responsive nowrap w-100">
                                                ...
                                                $sql ="SELECT *, SUM(monthly_price) AS tp FROM festive_pay_tbl WHERE account_officer='$accoff' AND category='$category' AND status='PAID'";
                                                $result2 = mysqli_query($conn,$sql);
                                                while($rows = mysqli_fetch_assoc($result2)){
                                                    $transdate = $rows['creation_date'];
                                                    $memberid = $rows['member_id'];
                                                    $membername = $rows['fullname'];
                                                    $category = $rows['category'];
                                                    $accoff = $rows['account_officer'];
                                                    $ppd = $rows['price_per_day'];
                                                    $mp = $rows['monthly_price'];
                                                    $tp = $rows['tp'];
                                                    $gt = $total + $tp;
                                                ?>
                                                <tr>
                                            ...
                                            <?php $i++; }}}
                                                ...
                                                <tfoot>
                                                    ...
                                                    <th></th>
                                                    <th>Total</th>
                                                    <th><strong><?php echo number_format($gt,2);?></strong></th>
                                                </tfoot>
