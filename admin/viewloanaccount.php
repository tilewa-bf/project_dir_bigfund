                                        ...
                                        <table id="datatable-buttons" class="table table-bordered dt-responsive nowrap w-100">
                                            ...
                                            if($_SERVER["REQUEST_METHOD"]=="POST"){
                                                  if(isset($_POST['submit'])){
                                                    ...
                                                    $sql = "SELECT p.loan_code,p.member_name,t.approved_date,t.maturity_date,t.tenor,p.applied_amount AS loanamount,t.total_payable AS totalpayable,p.approved_amount,SUM(p.repayment_amount) AS totalPaid,p.total_payable-SUM(p.repayment_amount) as Balance FROM rep_tbl AS p INNER JOIN loan_request AS t ON p.loan_code = t.loan_code WHERE p.tenor='WEEKLY' GROUP BY loan_code";
