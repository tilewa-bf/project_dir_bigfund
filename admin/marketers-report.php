...
<head>
    <meta charset="utf-8" />
    <title>Bigfund | Marketers Customers Savings Summary Report</title>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <h4 class="card-title" style="margin-top:30px;margin-bottom: -20px;">Marketers Customers Savings Summary Report</h4><br/>
                                ...
                                <tbody>
                                    ...
                                    if($_SERVER["REQUEST_METHOD"]=="POST"){
                                      if(isset($_POST['submit'])){
                                        ...
                                        $sql ="SELECT SUM(AMOUNT) AS marketerstotal,account_officer FROM deposit_savings_tbl WHERE trans_date BETWEEN '$from' AND '$to' AND entry_type='credit' AND trans_type='CD' AND account_number NOT LIKE '%100467%'AND account_number NOT LIKE '%100999%' AND status!='Processing' AND status2!='delete' GROUP BY account_officer";
                                        $query = mysqli_query($conn,$sql);

                                        while($rows = mysqli_fetch_assoc($query)){
                                            $accoff = $rows['account_officer'];
                                            $mt = $rows['marketerstotal'];
                                            $total = $total + $mt;
                                            ?>
                                            <tr>
                                                <td><?php echo $i;?></td>
                                                <td><?php echo $accoff;?></td>
                                                <td><?php echo number_format($mt,2);?></td>
                                            </tr>
                                            <?php $i++; }
                                        }}?>
                                    </tbody>
