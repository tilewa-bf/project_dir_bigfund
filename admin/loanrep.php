...
$savings_amount=$memberid = $loancode = $dbloanmembername =$dbmemberphone = $dbgroupname =$dbaccountofficer=$dbloanmemberid= $dbloancode= $dbloanpi = $dbloanappliedamount =$dbloanapprovedamount= $totalpayable =$dbloanapproveddate=$dbloanmaturitydate=$wkr=$noofweek=$dbloanbal="";
...
if(mysqli_num_rows($loanmemberquery)==1){
    ...
    //Select member from member details
    $sql = "SELECT * FROM member where status!='deleted' AND member_id='$dbloanmemberid'";
    $query = mysqli_query($conn,$sql);
    $rows = mysqli_fetch_assoc($query);
    if(isset($rows['phone'])){
        $dbmemberphone = $rows['phone'];
    }
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(isset($errors)){
        ...
        $sql = "INSERT INTO rep_tbl(rp_id,trans_id,member_id,loan_code,member_name,member_phone,group_name,account_officer,tenor,applied_amount,approved_amount,reverseamount,total_payable,repayment_amount,savings_amount,debit,credit,entry_type,status,narration,confirmed_by,created_by,creation_date,approved_date,maturity_date,date_time,updated_by,update_dt,status2) 
        VALUES (NULL,'$tid','$member_id','$loancode','$member_name','$phone','$groupname','$accoff','$tenor','$appliedamount','$approvedamount','$reverseamount','$totalpayable','$repayment_amount',0,'$debit','$credit','$entrytype','pending','$narration',NULL,'$dbname',now(),'$approveddate','$maturitydate','$dt',NULL,NULL,'undelete')";
...
','Loan Account',NULL,NULL,'$tenor','$appliedamount','$approvedamount','$reverseamount','$totalpayable','$savings_amount',NULL,'$credit','$debit',NULL,'pending','$narration',NULL,'$dbname',now(),'$dt',NULL,NULL,'undelete')";
        ...
        $sqlsav = "INSERT INTO `deposit_savings_tbl`(`mem_id`, `savings_type`, `trans_id`, `account_number`, `cust_name`, `account_officer`, `debit`, `credit`, `amount`, `narration`, `ref_no`, `trans_type`, `entry_type`, `pay_type`, `status`, `status2`, `status3`, `post_by`, `confirm_by`, `confirm_dt`, `trans_date`, `date_time`, `checked`) VALUES (null,'$tenor','$tid2','$member_id','$member_name','$accoff',0,0,0,'$narration','NA','CD','Credit','Cash','Processing','Undelete','unmove','$dbname',null,null,now(),'$dt',null)";
        $querysav = mysqli_query($conn,$sqlsav);

        //Insert the second leg into the depost_savings_tbl
        $sqldep = "INSERT INTO `deposit_savings_tbl`(`mem_id`,`savings_type`, `trans_id`, `account_number`, `cust_name`, `account_officer`,`debit`,`credit`, `amount`, `narration`,`ref_no`,`trans_type`, `entry_type`, `pay_type`, `status`, `status2`,`status3`, `post_by`, `confirm_by`,`confirm_dt`, `trans_date`, `date_time`,`checked`) VALUES (NULL,'$tenor','$tid2','100467','Savings Deposit','$accoff',0,0,0,'$narration','NA','CD','Debit','Cash','Processing','undelete','unmove','$dbname',NULL,NULL,now(),'$dt',null)";
        $querydep = mysqli_query($conn,$sqldep) or die($sqldep);

        /*** Insert into income table ***/
        $sqlinc2 = "INSERT INTO `income_tbl`(`inc_id`,`trans_id`, `account_no`, `account_name`,`cust_acc`, `cust_name`, `debit`, `credit`,`status`,`status2`,`post_by`,`confirmed_by`,`confirm_dt`,`trans_date`,`date_time`) VALUES (NULL,'$tid2','100467','Savings Deposit','$member_id','$member_name',0,0,'pending','undelete','$dbname',NULL,NULL,now(),'$dt')";
                                            ...
                                            <div class="row">
                                                 <div class="col-md-3">
                                                    <div class="mb-3">
                                                        ...
                                                        <input type="text" required="" name="name" value="<?php if(isset($dbloanmembername)){echo strtoupper($dbloanmembername);}?>" class="form-control" id="formrow-name-input" readonly>
                                                        <input hidden type="text" required="" name="phone" value="<?php if(isset($dbmemberphone)){echo $dbmemberphone;}?>" class="form-control" id="formrow-name-input" readonly>
                                                    </div>
                                                </div>
