                    ...
                    <div class="container-fluid">
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="card">
                                    <div class="card-body">
                                        <h4 class="card-title mb-4">Loan Repayment</h4>
                                        ...
                                        <table id="datatable-buttons" class="table table-bordered dt-responsive nowrap w-100">
                                            ...
                                            if($_SERVER["REQUEST_METHOD"]=="POST"){
                                                  if(isset($_POST['submit'])){
                                                    ...
                                                    $sql = "SELECT member_id,member_name, group_name,loan_code, account_officer, total_payable as tp, SUM(repayment_amount) as rep, SUM(reverseamount), total_payable - (SUM(repayment_amount)-SUM(reverseamount)) as balance FROM rep_tbl WHERE status!='pending' AND tenor='$savtype' AND group_name='$groupname' GROUP BY loan_code HAVING balance <>0;";
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <?php
                                                    $sqlpen = "select * FROM rep_tbl WHERE loan_code='$loancode'";
                                                    $querypen = mysqli_query($conn,$sqlpen);
                                                    $rows = mysqli_fetch_assoc($querypen);
                                                    $status = $rows['status'];
                                                     if($status=='pending'){?>
                                                        <a class="btn btn-info" onclick="alert('Ooops! You have pending loan repayment');">Loan Repayment</a>
                                                    <?php }else{?>

                                                        <a class="btn btn-info" href="loanrep.php?lid=<?php echo $rows['loan_code'];?>">Loan Repayment</a>
                                                    <?php }?>
