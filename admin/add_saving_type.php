...
if($_SERVER["REQUEST_METHOD"]=="POST"){
    if(isset($_POST['submit'])){
        ...
        if(empty($_POST['sc'])){
            ...
            $sc = test_input($_POST['sc']);
            $sc = strtoupper($sc);
        }
        if(empty($_POST['savint'])){
            $errors = "Savings Interest is empty";
            $alerttype= "danger";
            $icon = "block-helper";
        }else{
            $savint = test_input($_POST['savint']);
            $savint = strtoupper($savint);
        }
        ...
        $sql = "SELECT * FROM savings_type_tbl WHERE savings_name='".$sn."'";   
        ...
        if($numrows > 0){
            $alerttype= "danger";
            $icon = "block-helper";
            $msg = "Savings registered before!";
        }else{
            $sql2 = "INSERT INTO `savings_type_tbl`(`savings_id`, `savings_name`, `savings_code`, `savings_loan_int`, `savings_tenor`, `creation_date`, `creation_dt`, `created_by`, `updated_date`, `updated_dt`, `updated_by`, `status`) VALUES 
            (null,'$sn','$sc','$savint',null,now(),'$creation_dt','$dbname',null,null,null,'active')";
            ...
            if($query2){
                ...
                $alerttype= "success";
                $icon = "check-all";
                $msg = "Savings Registration Successful.";
                
            }else{
                $msg = "Savings Registration Not Successful";
            }
        }
        }
                                    ...
                                    <div class="card-body">
                                        ...
                                        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data">
                                            ...
                                            <div class="row">
                                                <div class="col-md-3">
                                                    <div class="mb-3">
                                                        <label for="formrow-group-input" class="form-label">SAVINGS NAME</label>
                                                        <input type="text" name="sn" class="form-control" id="formrow-groupname-input" placeholder="Enter Savings Name" required>
                                                    </div>
                                                </div>
                                                <div class="col-md-3">
                                                    <div class="mb-3">
                                                        <label for="formrow-leadername-input" class="form-label">SAVINGS CODE</label>
                                                        <input type="text" name="sc" class="form-control" id="formrow-leadername-input" placeholder="Enter Savings Code" required>
                                                    </div>
                                                </div>
                                                <div class="col-md-3">
                                                    <div class="mb-3">
                                                        <label for="formrow-leadername-input" class="form-label">SAVINGS INT</label>
                                                        <input type="text" name="savint" class="form-control" id="formrow-savint-input" placeholder="Enter Savings Interest" required>
                                                    </div>
                                                </div>
                                                <!-- <div class="col-md-3">
                                                    <div class="mb-3">
